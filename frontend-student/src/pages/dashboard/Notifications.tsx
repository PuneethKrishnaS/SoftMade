          ...
          <div className="divide-y divide-border/30">
             {NOTIFICATIONS.map((notif) => (
                <div key={notif.id} className={`flex items-start gap-5 p-5 hover:bg-secondary/40 transition-colors group cursor-pointer ${!notif.read ? 'bg-primary/5' : ''}`}>
                   <div className={`mt-0.5 p-3 rounded-2xl border shadow-sm group-hover:scale-110 transition-transform ${
                      notif.type === 'info' ? 'bg-blue-500/10 text-blue-500 border-blue-500/20' :
